with shop as (
    ...
    select * from {{ ref('stg_shopify__shop') }}
    ...
    select * from {{ ref('int_shopify__daily_orders') }}

), daily_abandoned_checkouts as (

    select * from {{ ref('int_shopify__daily_abandoned_checkouts') }}
    ...
    from calendar
    cross join shop
    where cast(shop.created_at as date) <= calendar.date_day
