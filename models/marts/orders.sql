with orders as (
    ...
    select
        order_id,
        source_relation,
        string_agg(distinct cast(value as string), ', ') as order_tags
    ...
    select
        ...
        string_agg(distinct cast(service as string), ', ') as fulfillment_services,
        string_agg(distinct cast(tracking_company as string), ', ') as tracking_companies,
    ...
    select
        orders.*,
        -- Parse shipping cost from total_shipping_price_set JSON
        coalesce(cast(json_extract_scalar(orders.total_shipping_price_set, '$.shop_money.amount') as numeric), 0) as shipping_cost,
