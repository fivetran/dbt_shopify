with calendar as (
    ...
    select *
    from analytics.public.assc_drop_cal
), orders_joined as (
    ...
    select 
        ...
        drop_calendar.drop_name,
        coalesce(count(distinct orders.order_id), 0) as order_count_in_month,
        coalesce(count(distinct orders.customer_id), 0) as shopped_in_month,
    ...
    left join drop_calendar on date(customer_calendar.first_order_timestamp) = drop_calendar.order_date
