...
| [`stg_shopify_gql__order_line`](https://fivetran.github.io/dbt_shopify/#!/model/model.shopify.stg_shopify_gql__order_line) | New columns |  | `unit_price_pres_amount`, `unit_price_pres_currency_code`, `unit_price_shop_amount`, `unit_price_shop_currency_code` | Exposes Shopify's own per-unit price fields (`originalUnitPriceSet`), synced by the connector but not previously mapped in this package. |
